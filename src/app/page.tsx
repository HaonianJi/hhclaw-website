    ...
    <div>
      ...
      <section ref={addRevealRef} className="scroll-reveal" style={{ borderTop: '1px solid var(--border)' }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <p className="mb-6" style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', lineHeight: 1.7 }}>
                ...
                <tbody>
                  {BENCHMARKS.map((b) => (
                    <tr key={b.name} style={{ cursor: 'default', background: b.ours ? 'rgba(255,107,53,0.05)' : undefined }}>
                      ...
                      <td style={{ textAlign: 'center' }}>{b.msc === 'true' ? '\u2705' : b.msc === 'partial' ? '\uD83D\uDFE1' : '\u274C'}</td>
                      <td style={{ textAlign: 'center' }}>{b.du === 'true' ? '\u2705' : b.du === 'partial' ? '\uD83D\uDFE1' : '\u274C'}</td>
                      <td style={{ textAlign: 'center' }}>{b.mu === 'true' ? '\u2705' : b.mu === 'partial' ? '\uD83D\uDFE1' : '\u274C'}</td>
                      <td style={{ textAlign: 'center' }}>{b.pref === 'true' ? '\u2705' : b.pref === 'partial' ? '\uD83D\uDFE1' : '\u274C'}</td>
...
const BENCHMARKS: { name: string; msc: string; du: string; mu: string; pref: string; verify: string; fw: number; scale: string; ours?: boolean }[] = [
